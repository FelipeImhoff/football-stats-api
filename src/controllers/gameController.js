...
import { createGame, getGameData, getGamesLinks } from '../models/gameModel.js'
...
async function processGames(links, batchSize, callback) {
  let result = []

  for (let i = 0; i < links.length; i += batchSize) {
    console.log(`${((i / links.length) * 100).toFixed(2)}% (${i}/${links.length})`)
    const batch = links.slice(i, i + batchSize)
    result = result.concat(await Promise.all(batch.map(async (link) => {
...
async function getTeamGames(request, response) {
  try {
...
async function getTeamGamesBySeason(request, response) {
  try {
    ...
    const result = Promise.all(gamesData)
...
async function processGamesSequentially(games) {
...
async function getGamesData(teamPage) {
  try {
    ...
    const playersStatsPromises = await processGames(filteredGames, 5, async (game) => await getGameData(game, __dirname, teamSelector))
...
async function getHomeManagers(request, response) {
  try {
...
async function getAwayManagers(request, response) {
  try {
    const data = await prisma.games.findMany({
      select: {
        id: true,
...
async function getHomeTeams(request, response) {
  try {
    const data = await prisma.games.findMany({
      select: {
        id: true,
...
async function getAwayTeams(request, response) {
  try {
    const data = await prisma.games.findMany({
      select: {
        id: true,
...
async function getGamesStats(request, response) {
